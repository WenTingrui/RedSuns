...
int main(void)
 {	
	...
	float uin1,uin2,uin3,y,left,
		 ...
		 B1[5]={0.711245406023192,-6.03816128313698,0.672422209392738,3.81628080873561,2.47034477623373},
	...
	Adc_Init();//ADC初始化
	uart_csb_init();//串口初始化
 	TIM3_PWM_Init(199,72-1);//电机初始化	 
	TIM2_PWM_Init(19999,72-1);//舵机初始化	
	TIM4_Cap_Init(50000,9-1);	//以500khz即0.002ms的频率计数 ，一次计500下
	TIM1_Int_Init(100-1,3840-1);//100Khz的计数频率，计数到10为0.1ms  

   	while(1)
	{			
		new_jianju=get_jianju();
		if(new_jianju >200&&new_jianju <3000)jianju =new_jianju;
